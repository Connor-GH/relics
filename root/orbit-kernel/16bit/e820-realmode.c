...
static void setMemMap(SMAP_entry_t *buf) {
    entries_buffer = buf;
}
...
static int __attribute__((noinline)) __attribute__((regparm(3))) detectMemory(int maxentries)
{
	uint32_t contID = 0;
...
//#if 0
	do
	{
        ...
        switch (smap->Type) {
        case '1': realmode_printk("(Memory)"); break;
        case '2': realmode_printk("(Reserved)"); break;
        case '3': realmode_printk("(ACPI)"); break;
        case '4': realmode_printk("(NVS)"); break;
        case '5': realmode_printk("(Unusable)"); break;
        case '6': realmode_printk("(Disabled)"); break;
        default: realmode_printk("(Unknown)"); break;
        }
        realmode_printk("\n");
	}
