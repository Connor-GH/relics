...
static int has_eflag(unsigned long mask)
{
    unsigned long f0, f1;
...
void cpuid_call(u32 id, u32 cnt, u32 *a, u32 *b, u32 *c,
        u32 *d) {
    __asm__ __volatile__
        ("movl $0, %%eax\t\n"
         "cpuid\t\n"
...
void set_cpu_vendor_name(void) {
    ...
    cpu_vendor_name[2] = (char)((cpu_vendor[0] & (bitmask << 16)) >> 16); // 23-16 bits
    cpu_vendor_name[3] = (char)((cpu_vendor[0] & (bitmask << 24)) >> 24); // 31-24 bits
