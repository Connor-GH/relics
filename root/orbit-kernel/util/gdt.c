...
static uint64_t gdt_descriptors[GDT_MAX_ENTRIES];
...
static void fill_gdt_descriptor_item(size_t vector, uint32_t base,
	...
	gdt_descriptors[vector] = set_several_bits(gdt_descriptors[vector], 0, 16, (uint16_t)(limit & 0x0000FFFF));
	gdt_descriptors[vector] = set_several_bits(gdt_descriptors[vector], 16, 40, base & 0xFFFFFF);
	gdt_descriptors[vector] = set_several_bits(gdt_descriptors[vector], 40, 48, access);
	gdt_descriptors[vector] = set_several_bits(gdt_descriptors[vector], 48, 52, (limit >> 16) & 0xF);
	gdt_descriptors[vector] = set_several_bits(gdt_descriptors[vector], 52, 56, flags & 0xF);
	gdt_descriptors[vector] = set_several_bits(gdt_descriptors[vector], 56, 64, (uint8_t)(base >> 24));
}
