...
static inline void
list_add_(struct ll_head *n, struct ll_head *prev, struct ll_head *next)
{
	next->prev = n;
	n->next = next;
	n->prev = prev;
...
static inline void
list_add(struct ll_head *n, struct ll_head *head)
{
	list_add_(n, head, head->next);
}

static inline void
list_add_tail(struct ll_head *n, struct ll_head *head)
{
	list_add_(n, head->prev, head);
}

//void list_insert(struct ll_head *between, struct ll_head *left, struct ll_head *right) {
//}
#define list_insert list_add_
#pragma mark - Delete -

static inline void
list_del_(struct ll_head *prev, struct ll_head *next)
{
	next->prev = prev;
	prev->next = next;
}
