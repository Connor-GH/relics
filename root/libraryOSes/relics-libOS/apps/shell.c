...
static void echo_builtin(char *buf) {
...
static void cursor_builtin(const char *buf) {
    /* command goes as follows:
     * COMMAND -E/D
     */
    if (buf[7] == '-') {
        switch (buf[8]) {
        case 'E': enable_cursor(1,1); break;
        case 'D': disable_cursor(); break;
        default: break;
        }
   } else {
       printk("Options:\n"
...
void relics_shell(const char *prompt) {
